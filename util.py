...
def uploadToTikTok(video, session):
    ...
    response_json = r.json()  # Get the JSON response
    print("Response JSON:", response_json)  # Debug: Print the whole response

    # Adjusted to use 'video_token_v5' instead of 'Result'
    if 'video_token_v5' not in response_json:
        print("Error: 'video_token_v5' key not found in response. Response content:", response_json)
        return False

    video_token_v5 = response_json['video_token_v5']

    # Extract necessary details from video_token_v5
    access_key = video_token_v5.get('access_key_id')
    ...
    if not all([access_key, secret_key, session_token]):
        print(f"Error: Missing required token details. access_key: {access_key}, secret_key: {secret_key}, session_token: {session_token}")
        return False
